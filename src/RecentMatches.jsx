...
const normalizeRecentMatches = (matches, mode) =>
  (Array.isArray(matches) ? matches : [])
    .map((match) => {
      ...
      const sourceValue = [match?.source, match?.match_source, match?.queue]
        .find((value) => value !== undefined && value !== null && String(value).trim().length > 0);
      const tournamentValue = match?.tournament ?? match?.is_tournament;
      const friendValue = match?.friend ?? match?.is_friend;
      const lobbyValue = match?.lobby ?? match?.is_lobby;
      ...
      return {
        ...
        sourceValue:
          sourceValue === undefined || sourceValue === null || String(sourceValue).trim().length === 0
            ...
            : String(sourceValue),
        tournamentValue:
          tournamentValue === undefined || tournamentValue === null ? "—" : String(tournamentValue),
        friendValue: friendValue === undefined || friendValue === null ? "—" : String(friendValue),
        lobbyValue: lobbyValue === undefined || lobbyValue === null ? "—" : String(lobbyValue),
      };
    })
            ...
            return (
              ...
              >
                ...
                <div className="matchCardMeta">
                  ...
                  <span>
                    Source {match.sourceValue} • tournament {match.tournamentValue} • friend {match.friendValue} • lobby {match.lobbyValue}
                  </span>
                </div>
                {isExpanded ? (
