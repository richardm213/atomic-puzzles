...
const matchModeOptions = ["blitz", "bullet", "all"];
...
const normalizeAllMatches = (matches, mode) => {
  return (Array.isArray(matches) ? matches : [])
    .map((match) => {
      ...
      return {
        ...
        scoreA,
        scoreB,
        score: `${formatScore(scoreA)}-${formatScore(scoreB)}`,
        ...
        minPlayerRating:
          Number.isFinite(playerARating) && Number.isFinite(playerBRating)
            ? Math.min(playerARating, playerBRating)
            : null,
    ...
    <div className="rankingsPage">
      <div className="panel rankingsPanel">
        ...
        <div className="profileBackLinkWrap">
          <a className="rankingLink" href="/matches">
            View latest matches →
          </a>
        </div>
  ...
  useEffect(() => {
    const loadMatches = async () => {
      ...
      try {
        ...
        const normalized =
          selectedMode === "all"
            ? normalizeAllMatches(loaded, "mixed")
            : normalizeAllMatches(loaded, selectedMode);
  ...
  return (
    <div className="rankingsPage">
      <div className="panel rankingsPanel">
        ...
        <div className="controls rankingsControls profileControls">
          <label htmlFor="matches-mode-select">
            Mode
            <select
              id="matches-mode-select"
              value={selectedMode}
              onChange={(event) => setSelectedMode(event.target.value)}
            >
        ...
        <div className="opponentRatingFilter">
          <label htmlFor="matches-average-rating-min">
        ...
        <div className="opponentRatingFilter">
          <label htmlFor="matches-player-rating-min">
        ...
        <div className="opponentRatingFilter">
          <label htmlFor="matches-length-min">
            Match length range: {matchLengthMin} - {matchLengthMax}
          </label>
          <div className="dualRangeSlider">
            <div className="dualRangeTrack" />
        ...
        <div className="rankingsMeta">
          <span>Showing latest matches</span>
          <span>
            {filteredMatches.length} filtered / {matches.length} total
          </span>
        </div>
...
export const RankingsPage = ({ username = "", isMatchesRoute = false }) => {
  if (isMatchesRoute) {
    return <MatchesView />;
  }
