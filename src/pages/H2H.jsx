...
export const H2HPage = () => {
  ...
  const timeControlOptions = useMemo(() => {
    const set = new Set(filteredMatches.map((match) => match.timeControl));
    return [...set].sort((a, b) => {
      const [aBase, aInc] = String(a).split("+").map(Number);
      const [bBase, bInc] = String(b).split("+").map(Number);
  ...
  const handleSearch = async () => {
    ...
    try {
      const [blitzRaw, bulletRaw, firstRatings, secondRatings] = await Promise.all([
        loadRawMatchesByMode("blitz", { filters: { username: first } }),
        loadRawMatchesByMode("bullet", { filters: { username: first } }),
  ...
  const renderPlayerPanel = (name, snapshot) => (
        ...
        {hasSearched && loadedPlayer1 && loadedPlayer2 ? (
          <>
            <div className="h2hSplitLayout">
              {renderPlayerPanel(loadedPlayer1, player1Snapshot)}
              {renderPlayerPanel(loadedPlayer2, player2Snapshot)}
            </div>

            <div className="h2hScoreBoard">
              <h2>H2H Score</h2>
              <p>
                Blitz: <strong>{loadedPlayer1}</strong> {blitzScore.playerA} - {blitzScore.playerB}
                <strong>{loadedPlayer2}</strong>
                {blitzScore.draws ? ` (drawn matches: ${blitzScore.draws})` : ""}
              </p>
              <p>
                Bullet: <strong>{loadedPlayer1}</strong> {bulletScore.playerA} - {bulletScore.playerB}
                <strong>{loadedPlayer2}</strong>
                {bulletScore.draws ? ` (drawn matches: ${bulletScore.draws})` : ""}
              </p>
              {scoreByTc.length ? (
