...
const comparePuzzles = (left: { puzzleId?: number | string }, right: { puzzleId?: number | string }): number => {
  const leftId = Number(left?.puzzleId ?? 0);
  const rightId = Number(right?.puzzleId ?? 0);
  return rightId - leftId;
};

const buildAuthorSet = (profileAliasEntry: import("../../lib/supabase/supabaseAliases").MergedAliasRow | null | undefined, fallbackUsername: string): Set<string> => {
...
export const PuzzleContributionsPage = ({ username = "" }: { username?: string }) => {
