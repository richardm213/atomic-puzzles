...
export class PuzzleSubmissionRepository {
  ...
  async publish(username: string, puzzle: QueuedPuzzleSubmission): Promise<number> {
    const { error: userError } = await this.supabase
      .from("users")
      .upsert({ username }, { onConflict: "username", ignoreDuplicates: true });
    if (userError) throw new Error(`Unable to verify account: ${userError.message}`);

    const { data, error } = await this.supabase.rpc("publish_approved_puzzle_submission", {
      p_fen: puzzle.fen,
      p_solution: puzzle.solution,
      p_event: puzzle.event,
      p_explanation: puzzle.explanation,
    ...
    if (!Number.isSafeInteger(puzzleId) || puzzleId < 1) {
      throw new Error("Unable to create puzzle: no puzzle id was returned.");
    }
    return puzzleId;
  }
}
